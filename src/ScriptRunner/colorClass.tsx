...
export const checkRgbColorValidity = (rgbColor: unknown) => {
  ...
  for (const channel of RGBA_CHANNELS) {
    ...
    if (typeof channelValue !== `number`) {
      throw new TypeError(`The ${channel} channel is not a number (received ${typeof channelValue})`);
    }
    ...
    if (
      ...
      throw new TypeError(`The ${channel} channel is out of the range [0, 255] (received ${channelValue})`);
    }
  }
};
...
const checkHslColorValidity = (hslColor: unknown) => {
  ...
  for (const channel of HSLA_CHANNELS) {
    ...
    if (typeof channelValue !== `number`) {
      throw new TypeError(`The ${channelValue} channel is not a number (received ${typeof channelValue})`);
    }
  }
  ...
  if (MAX_HUE_VALUE_EXCLUDING <= hue || hue < MIN_HUE_VALUE) {
    throw new TypeError(`The hue channel is out of the range [0.0, 360.0) (received ${hue})`);
  }

  if (
    MAX_RGBA_CHANNEL_VALUE < alpha || alpha < MIN_RGBA_CHANNEL_VALUE
    || alpha % 1 !== 0
  ) {
    throw new TypeError(`The alpha channel is out of the range [0, 255] (received ${alpha})`);
  }
  ...
  for (const channel of [`saturation`, `lightness`]) {
    ...
    if (
      ...
      throw new TypeError(`The ${channel} channel is out of the range [0.0, 1.0] (received ${channelValue})`);
    }
  }
};
...
const normalizeRgbColor = (rgbColor: unknown, ...excess: unknown[]) => {
  if (excess.length > 0) {
    throw new TypeError(`Color.normalizeRgbColor expected 1 argument, received ${excess.length + 1}`);
  }
...
const normalizeHslColor = (hslColor: unknown, ...excess: unknown[]) => {
  if (excess.length > 0) {
    throw new TypeError(`Color.normalizeHslColor expected 1 argument, received ${excess.length + 1}`);
  }
...
export default {
  ...
  brightness(color: Color, ...excess: unknown[]) {
    if (excess.length > 0) {
      throw new TypeError(`Color.brightness expected 1 argument, received ${excess.length + 1}`);
    }
  ...
  parseHex(hex: string, ...excess: unknown[]) {
    if (excess.length > 0) {
      throw new TypeError(`Color.parseHex expected 1 argument, received ${excess.length + 1}`);
    }
  ...
  createHex(color: Color, ...excess: unknown[]) {
    if (excess.length > 0) {
      throw new TypeError(`Color.createHex expected 1 argument, received ${excess.length + 1}`);
    }
  ...
  rgbToHsl(color: Color, ...excess: unknown[]) {
    if (excess.length > 0) {
      throw new TypeError(`Color.rgbToHsl expected 1 argument, received ${excess.length + 1}`);
    }
  ...
  hslToRgb(color: HslColor, ...excess: unknown[]) {
    if (excess.length > 0) {
      throw new TypeError(`Color.hslToRgb expected 1 argument, received ${excess.length + 1}`);
    }
